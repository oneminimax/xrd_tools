'''
The atomic form factors were taken from the International Tables for Crystallography: 
http://it.iucr.org/Cb/ch6o1v0001/.
'''

import numpy as np

ITCFormFactorCoefDict = {
    ...
    'Cf'   :(36.9185 ,  0.437533,   25.1995 ,   3.00775 ,   18.3317 ,   12.4044 ,   4.24391 ,   83.7881 ,   13.267),
}

def ITCFct(label):

    if label in ITCFormFactorCoefDict:
        coef = ITCFormFactorCoefDict [label]
        A = np.array(coef[0:8:2])
        B = np.array(coef[1:8:2])
        c = coef[8]

        print(A,B,c)

        return lambda q : np.sum(A * np.exp(-B*(q/10/(4*np.pi))**2)) + c
